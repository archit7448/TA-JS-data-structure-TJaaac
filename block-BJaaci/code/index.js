...
function genreMovie(n){
  let index = n -1
  for(let i = 0 ; i<movies[index].genres.length ; i = i+1 ){
    console.log(movies[index].genres[i])
  }
}
...
if(Number(movies[0].year) > 2017 && Number(movies[1].year) > 2017 && Number(movies[2].year) > 2017 == true){
   console.log( true)
}else{
  console.log(false)
}
...
for(let i = 0 ; i < movies.length; i = i+1){
  console.log(`${movies} ,${movies[i].actors.length}`)
}

// 26. Log the number of genres in all three movies one by one

for(let i = 0 ; i < movies.length; i = i+1){
  console.log(`${movies[i].title} ,${movies[i].genres.length}`)
}
